...
double mycost(unsigned n, const double *U, double *null_1, void *null_2) //unsigned n, const double *U
{
  double ts = 0.002;  // step time
  // double XX[3] = {x_p_global, y_p_global, theta0_global}; // point p states feedbacks from sensors
  double XX[3] = {x_c_global, y_c_global, theta0_global}; // point center "c" states feedback from sensors
  ...
  // (1) "B" matrix without linear term
  B[0] = cos(XX[2]);
  B[1] = -sin(XX[2]);
  B[2] = 0;
  B[3] = sin(XX[2]);
  ...
  // (2) "B" matrix with linear term --- chunming
  ...
  // (3) "B" matrix with linear term --- pranav
  ...
  mul[2] = X_Xref[0]*Q[2]+X_Xref[1]*Q[5]+X_Xref[2]*Q[8];
  cost_state = X_Xref[0]*mul[0]+mul[1]*X_Xref[1]+mul[2]*X_Xref[2];
  // cost_controls = U[3]*R[0]*U[3] + U[4]*R[1]*U[4] + U[5]*R[2]*U[5]; // chunming --- added for tracking problem
  cost = cost + cost_state + cost_controls;
  ...
  // mul[2] = X_Xref[0]*Q[2]+X_Xref[1]*Q[5]+X_Xref[2]*Q[8];
  // cost_state = X_Xref[0]*mul[0]+mul[1]*X_Xref[1]+mul[2]*X_Xref[2];
  // // cost_controls = U[6]*R[0]*U[6] + U[7]*R[1]*U[7] + U[8]*R[2]*U[8]; // chunming --- added for tracking problem
  // cost = cost + cost_state + cost_controls;
  ...
  // mul[2] = X_Xref[0]*Q[2]+X_Xref[1]*Q[5]+X_Xref[2]*Q[8];
  // cost_state = X_Xref[0]*mul[0]+mul[1]*X_Xref[1]+mul[2]*X_Xref[2];
  // // cost_controls = U[9]*R[0]*U[9] + U[10]*R[1]*U[10] + U[11]*R[2]*U[11]; // chunming --- added for tracking problem
  // cost = cost + cost_state + cost_controls;
  ...
  for (j=0;j<4;j++)
  {
    ...
    if (ttime >= ts && ttime <= ts+tend) //ensures trajectory is provided after ts seconds
    {
      if (prev_step < step_no) //ensures values are set once per step
      {
        prev_step = step_no;
        double x_ref, y_ref;
        double x_ref_temp, y_ref_temp; // chunming --- use for testing
        ...
        //get trajectory reference --- pranav's "8" curve
        // double x_center = -a;
        // double y_center = 0;
        ...
        //                 &x_ref_temp, &y_ref_temp);

        // get trajectory reference --- chunming simple poly
        // get_trajectory(ttime,ts,tend,&x_ref,&y_ref,&theta_ref,
        //                              &xdot_ref, &ydot_ref, &thetadot_ref);

        // chunming --- get circle trajectory
        ...
        x_ref_global = x_ref;
        x_ref_global_temp = x_ref_temp; // chunming --- use for testing
        xdot_ref_global = xdot_ref;
        y_ref_global = y_ref;
        y_ref_global_temp = y_ref_temp; // chunming --- use for testing
        ...
        // (point p) update the most global states variable to let them go inside the mycost function;
        x_p_global = x_p;
        y_p_global = y_p;
        theta0_global = theta0;
        // (point c) update the most global states variable to let them go inside the mycost function;
        x_c_global = x_c;
        y_c_global = y_c;
        // x_c_global = x_c + 1; // chunming --- want to move the curve to positive area
        // y_c_global = y_c + 1; // chunming --- want to move the curve to positive area
        ...
        int flag_mpc = 0; // "1" using pranav's controller, "2" using chunming's controller
        if (flag_mpc!=0) // pranav's feedback linearization controller
        {
       ...
       else
       {
          ...
          double inputs_vx; // feedforward term
          double inputs_vy;// feedforward term
          double inputs_omega;// feedforward term
          ...
          // (1) final inputs/controls in each steps just with the error compensate
          // vx = inputs_vx;
          // vy = inputs_vy;
          // omega = inputs_omega;
        
          // (2) final inputs/controls in each steps with the feedforward term
          // vx = xdot_ref_global + inputs_vx;
          // vy = ydot_ref_global + inputs_vy;
          // omega = theta0dot_ref_global + inputs_omega;
          // omega = 0;

          // (3) only have the reference profile
          // vx = xdot_ref_global;
          // vy = ydot_ref_global;
          // omega = 0;

          // (4) find out that mpc always exhaust the controls to the limitation boundary, if we have a gain to regulater the controls
          double kp_mpc = 0.01;
...
// ********* back up some unused function in this code iteration *************************//
// void read_data()
// {
//     int i, j;
