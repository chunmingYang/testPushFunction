...
void init_save_data()
{
  ...
  fprintf(fid2,"WorldVelX,WorldVelY,WorldVelZ,"); // additional comma behind
  
  // chunming --- added to save the nlopt computational time
  fprintf(fid2, "NlopyStepSolveTime,");

  // chunming --- save the reference trajectory
  fprintf(fid2, "referenceX,");
  fprintf(fid2, "referenceY,");
  fprintf(fid2, "referenceTheta,");

  // chunming --- save actual yaw angle
  fprintf(fid2, "yaw");
...
void save_data(int motiontime, const LowState *state,const LowCmd *cmd,const RobotState *modelstate, double nloptSpeed, double x_c_global, double y_c_global)
{
      int i;
      ...
      // chunming --- added to save the nlop step conputation time
      fprintf(fid2,"%f,",nloptSpeed);

      // chunming --- save the reference traj.
      fprintf(fid2,"%f,",x_ref_global);
      fprintf(fid2,"%f,",y_ref_global);
      fprintf(fid2,"%f,",theta0_ref_global);

      // chunming --- save the actual yaw angle
      fprintf(fid2,"%f",theta0_global);
