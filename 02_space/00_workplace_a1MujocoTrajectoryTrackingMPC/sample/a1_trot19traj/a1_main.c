// chunming --- global variables to store the information from local function to be used outside the function
double x_ref_global;  
...
// chunming --- added to save the nlopt step computation speed
double nloptSpeed;

// chunming --- warm start format
double u_warmstart1;
double u_warmstart2;
double u_warmstart3;
double u_warmstart4;
...
void RobotControl(const mjModel* m, mjData* d)
{
  ...
  {
    my_controller(motiontime,quat_act,omega_act,q_act,u_act,tau,Kp,Kd,q_ref,u_ref, tauEst,footForce,bodyWorldPos);
